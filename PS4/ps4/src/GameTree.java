...
public class GameTree {
     ...
     */
    int helper(TreeNode node, Player player) {
        if (node == null) {
            ...
            if (player == Player.ONE) {
                ...
                for (TreeNode child : node.children) {
                    int value = helper(child, this.other(player));
    ...
    int findValue() {
        ...
        Player nextPlayer = numOne <= numTwo ? Player.ONE : Player.TWO;
        return helper(root, nextPlayer);
    }

    // Simple main for testing purposes
    public static void main(String[] args) {
        GameTree tree = new GameTree();
        tree.readTree("games/tictac_2_empty.txt");
